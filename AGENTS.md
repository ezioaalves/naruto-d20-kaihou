...
```
naruto-d20-kaihou/
├── module.json                       Foundry module manifest (single esmodule entry)
├── package.json                      npm scripts + devDependencies
├── CLAUDE.md                         agent guidance (read first)
├── AGENTS.md                         this file
├── scripts/
│   ├── kaihou.mjs                    module entry point
│   ├── apps/wizard/                  ApplicationV2 20 Questions wizard
│   ├── grants/                       item-grants engine + school/occupation auto-apply
│   └── theme/                        zen theme layer
├── generators/                       Python vault→pack-source generators
├── packs/_source/<pack>/             generated JSON, COMMITTED (villages, questions, schools, occupations)
├── packs/<pack>/                     compiled LevelDB, gitignored
├── data/skill-key-mapping.json       vault slug → PF1e key
├── templates/apps/tqw-v2/            wizard Handlebars partials
├── scss/ → styles/                   Sass sources → committed compiled CSS
├── lang/                             en + pt-BR string tables
├── tests/                            pytest (generators) + vitest (tests/wizard/)
...
npm run generate-villages             regenerate pack JSON from vault (also: -questions, -schools, -occupations)
npm run validate:compendia            pack-source JSON schema check
npm test                              pytest + vitest + CSS freshness
npm run lint                          ESLint + Stylelint + Prettier
npm run pack:all                      compile packs/_source → LevelDB
